...
@login_required
def quiz():
    if not session.get('welcome', False):
        flash(f'All the best {current_user.username}!')
...
@login_required
def Englishquiz():
    if not session.get('hello', False):
        flash(f'All the best {current_user.username}!')
        session['hello'] = True
...
def add_question():
    try:
        ...
        questions_path = os.path.join(app.static_folder, 'questions.txt')
        with open(questions_path, 'a') as file:
